...
mock_missions = [
    ...
    {"id": 2, "content": "탄소배출을 위해 우리가 할 수 있는 행동", "isCompleted": False},
]
...
async def get_mission(id: int, token: str = Depends(oauth2_scheme)):
...
async def get_mission_list(token: str = Depends(oauth2_scheme)):
    payload = decode_access_token(token)
    user_id = payload.get("sub")
    if not user_id:
        raise HTTPException(status_code=403, detail="Invalid token payload")
...
async def complete_mission(request: MissionCompleteRequest, token: str = Depends(oauth2_scheme)):
    payload = decode_access_token(token)
    user_id = payload.get("sub")
    if not user_id:
        raise HTTPException(status_code=403, detail="Invalid token payload")
